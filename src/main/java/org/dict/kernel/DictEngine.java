...
public class DictEngine implements IDictEngine {
	...
	public IAnswer[] define(String db, String word) {
		IAnswer[] ans = defineMatch(db, word, null, true,
	 ...
	 */
	public IAnswer[] defineMatch(String db, String word, String pos,
			boolean define, int strategy) {
		if (pos != null) {
			...
			return new IAnswer[] { d.defineMatch(idx, define,
					IDatabase.STRATEGY_NONE) };
		}
		if (word == null)
			return null;
	...
	public IAnswer[] lookup(IRequest req) {
		...
		for (int i = 0; i < db.length; i++) {
			try {
				...
				sb.append("</pre>");
				IAnswer a = new Answer(this.getDatabases()[0], word, -1, sb
						.toString());
