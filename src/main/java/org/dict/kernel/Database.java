...
public class Database implements IDatabase {
	...
	IWordList findExactMatches(String word, int pos) {
		if (pos < 0) {
			...
			IWordPosition[] arr = new IWordPosition[] { new WordPosition(word,
					pos) };
	...
	public String initialize(DatabaseConfiguration dc) throws Exception {
		...
		if (dataFile.getName().toLowerCase().endsWith(".dz")) {
			acc = new org.dict.zip.DictZipDataAccessor(dataFile
					.getAbsolutePath());
		} else {
		...
		try {
			if (formatter != null) {
				fHTMLPrinter = (IAnswerPrinter) Class.forName(formatter)
						.newInstance();
			}
		} catch (Throwable t) {
		...
		try {
			if (formatter != null) {
				fPlainPrinter = (PlainPrinter) Class.forName(formatter)
						.newInstance();
			}
		} catch (Throwable t) {
			sb.append("\nCannot init PlainPrinter: " + t);
		}
		sb.append("\nPlain Printer used for formatting output: "
				+ fPlainPrinter);
	...
	public static IDatabase createDatabase(DatabaseConfiguration dc)
			throws Exception {
