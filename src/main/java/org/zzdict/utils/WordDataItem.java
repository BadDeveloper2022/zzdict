...
/**
 * WordDataItem class is the abstract class of all word data item.
 * Every WordDataIteam implement convertWordDataToHtml() method, 
 * because we use html engine to display the word explanation.
 * 
 * @author zzh
 *
 */
...
class PureTextWordDataItem extends WordDataItem{
	...
	/**
	 * concreted convert word data to html implementation
	 * We should mark different part of word explanation, 
	 * like pronunciation, word sentence samples etc
	 * Later, we will apply css to this html code
	 */
...
enum WordDataType{
	...
	CHINESE_YINBIAO_OR_JAPANESE_KANA('y',"Chinese YinBiao or Japanese KANA. " +
			"The data should be a utf-8 string ending with '\0'."),
	POWERWORD_UTF8_XML('k',"KingSoft PowerWord's data. The data is a utf-8 string ending with '\0'. " +
			"It is in XML format."),
	MEDIA_WIKI_MAKRUP_UTF8_TEXT('w',"MediaWiki markup language. " +
			"See http://meta.wikimedia.org/wiki/Help:Editing#The_wiki_markup"),
	HTML('h',"Html codes."),
	WORDNET('n',"WordNet data."),
	RESOURCE_FILES('r',"Resource file list. " +
			"The content can be: " +
	...
	WAVE_FILE('W',"wav file. " +
			"The data begins with a network byte-ordered guint32 to identify the wav " +
			"file's size, immediately followed by the file's content."),
	PICTURE_FILE('P',"Picture file. " +
			"The data begins with a network byte-ordered guint32 to identify the picture " +
			"file's size, immediately followed by the file's content."),
	RESERVED('X',"this type identifier is reserved for experimental extensions.")
	;
	
	/**
	 * type char is just the char stored in the beginning of each word data section in dict file
	 */
	private char type;
	
	/**
	 * explanation of this type word data item, we store it here just for later usage
	 */
	private String comment;

	/**
	 * We use a static inside map to store mapping of the char to WordDataType related to this char
	 */
	 ...
	 * @param type given type character
