...
public class NetUtils {
	...
	static String convertTag(URL url, String s) {
		try {
			...
			int l;
			for (l = k + 1; !isSpace(s1.charAt(l)) && s1.charAt(l) != '"'
					&& s1.charAt(l) != '>'; l++)
				;
			...
			// System.out.println("part1: "+s1.substring(0,
			// k)+" ... part2: "+s1.substring(l));
			// String s3 = s2.toLowerCase();
			if (link) {
				s = s1.substring(0, k) + "/redir?url=" + URLEncoder.encode(s2)
						+ s1.substring(l, s1.length());
				// s = s1.substring(0, k) + "/redir/" + URLEncoder.encode(s2) +
				// s1.substring(l, s1.length());
			} else {
	...
	static boolean isRemoteTextLink(String s) {
		if (s.lastIndexOf(".htm") >= 0 || s.lastIndexOf(".txt") >= 0
				|| s.lastIndexOf(".shtml") >= 0) {
			return true;
		}
		if (s.endsWith("/")) {
			return true;
