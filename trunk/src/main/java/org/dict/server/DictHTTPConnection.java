...
public class DictHTTPConnection implements Runnable {
	...
	public static String[] INDEXES = new String[] { "index.html", "index.htm",
			"default.htm" };
		...
		if ("shutdown".equals(cmd)) {
			...
			if (password != null && password.hashCode() == k) {
				Logger.getInstance().log(
						"Shutdown command received at " + new java.util.Date());
				Logger.getInstance().flush();
				try {
					String msg = getHead("HTTP/1.0 200 OK", "")
							+ "Server shut down!";
		...
		try {
			...
			try {
				...
				if (!f.exists() && name.endsWith("/index.html")) {
					String list = listDir(new File(tmp.substring(0, tmp
							.length() - 10)));
					file = new BufferedInputStream(new StringBufferInputStream(
							list));
					len = list.length();
				} else {
					file = new BufferedInputStream(new FileInputStream(f));
	...
	private String listDir(File f) {
		...
		for (int i = 0; i < a.length; i++) {
			if (new File(f, a[i]).isDirectory()) {
				sb.append("[DIR] <a href=\"" + URLEncoder.encode(a[i]) + "/\">"
						+ a[i] + "</a><br>\n");
			}
		}
		for (int i = 0; i < a.length; i++) {
			if (new File(f, a[i]).isFile()) {
				sb.append("<a href=\"" + URLEncoder.encode(a[i]) + "\">" + a[i]
						+ "</a><br>\n");
			}
		}
		sb.append("</body></html>");
	...
	public void run() {
		...
		try {
			...
			// OutputStreamWriter(theConnection.getOutputStream(), "UTF8"));
			...
			OutputStream os = new BufferedOutputStream(fSocket
					.getOutputStream());
			serve(os, uri, params);
			os.flush();
		} catch (Throwable e) {
	...
	private void serve(OutputStream os, String uri, String params)
			throws IOException {
