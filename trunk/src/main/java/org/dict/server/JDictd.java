...
public class JDictd implements Runnable {
	...
	public JDictd(ServerSocket dictdSocket, ServerSocket httpSocket,
			IDictEngine engine) {
	...
	public static void createForm(File f, IDatabase[] dbs) {
		...
		try {
			...
			out.println(head);
			out
					.println("<select multiple name=\"db\" size=" + dbs.length
							+ ">");
			for (int i = 0; i < dbs.length; i++) {
				String db = dbs[i].getID();
				if (i == 0) {
					out.println("  <option value=" + db + " selected>"
							+ dbs[i].getName());
				} else {
	...
	public static void listen(String[] args) {
		...
		try {
			int httpPort = Integer
					.parseInt(prop.getProperty(HTTP_PORT, "2626"));
			if (httpPort > 0) {
				...
				trace(new Date().toString() + ": HTTP server started at "
						+ httpPort);
			} else {
	...
	public static void main(String args[]) {
		if (args.length == 0) {
			System.out
					.println("Usage: java ...JDictd configFile [configFile ...]");
			System.exit(0);
		}
		listen(args);
	}
