...
/**
 * Insert the type's description here. Creation date: (24.02.2002 14:44:28)
 * 
 * @author:
 */
public class HTMLPrinter implements IAnswerPrinter {
	...
	public static void printAnswers(IDictEngine e, IRequest req, IAnswer[] a,
			boolean matches, PrintWriter out) throws IOException {
		if (HEAD == null) {
			initialize();
		}
		out.println(HEAD);
		if (a == null || a.length == 0) {
			...
			out
					.println("<p>The specified database is not valid, or word not found in the database.");
			out.println("<p><a href=\"index.html\">Please go to main page</a>");
			out
					.println("<form name=trans><input name=word type=hidden value=\"\"></form>");
			out.println(TAIL);
			return;
		}
		...
		out.println("<form name=trans><input name=word type=hidden value=\""
				+ a[0].getKey() + "\"></form>");
		out.println(TAIL);
	}

	public static void printDefaultForm(IDictEngine e, IRequest req,
		...
		for (int i = 0; i < dbs.length; i++) {
			...
			out.println("  <option value=" + getOption(dbs[i].getID(), ids)
					+ ">" + db);
		}
	...
	protected void addLink(StringBuffer sb, String line, String word,
			String base) {
		sb.append(line);
	}
	...
	public String makeLink(String key, String def, String base) {
		if (def == null)
			return "";
		String delim = "{}";
		java.util.StringTokenizer st = new java.util.StringTokenizer(def,
				delim, true);
	...
	public void printAnswer(IRequest req, IAnswer a, boolean matches,
			PrintWriter out) throws IOException {
	...
	protected void printDefinition(IAnswer a, PrintWriter out, String base)
			throws IOException {
	...
	protected void printWordList(IWordList ls, int pos, StringBuffer sb,
			String posBase) {
