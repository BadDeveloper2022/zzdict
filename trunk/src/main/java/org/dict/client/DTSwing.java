...
public class DTSwing {
	...
	private void initialize() {
		...
		fontChoice = new JComboBox(new String[] { "Arial", "Verdana",
	...
	protected void lookup(String s) {
		...
		IAnswer[] a = engine.defineMatch(dbID, s, null, true,
				IDatabase.STRATEGY_NONE);
		StringWriter w = new StringWriter();
		PrintWriter out = new PrintWriter(w);
		IRequest req = new SimpleRequest("", "word=" + s);
		try {
			org.dict.kernel.PlainPrinter
					.printAnswers(engine, req, a, true, out);
			out.flush();
		} catch (Exception e) {
