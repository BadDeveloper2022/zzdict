...
public class TDDT extends JFrame {
	...
	class ScrollAL implements AdjustmentListener {
		...
		public void adjustmentValueChanged(AdjustmentEvent e) {
			...
			IAnswer[] a = engine.defineMatch(dbID, null, "" + pos, false,
					IDatabase.STRATEGY_NONE);
			showMatches(a, false);
		}
	}
	...
	static final String[] FONT_NAMES = new String[] { "Verdana", "Arial",
			"Courier New", "Arial Unicode MS" };
	static final String[] FONT_SIZES = new String[] { "10", "12", "14", "16",
			"20", "24" };
	...
	private void addDatabases(String cfg) {
		DatabaseConfiguration[] cfgs = DatabaseConfiguration
	...
	protected void findMatches() {
		String s = getInputField().getText();
		IAnswer[] a = engine.defineMatch("*", s, null, false,
				IDatabase.STRATEGY_NONE);
		showMatches(a, true);
	}
	...
	protected void lookup(String s, boolean append) {
		...
		// getFrame().toFront();
		IAnswer[] a = engine.defineMatch("*", s, null, true,
				IDatabase.STRATEGY_NONE);
		StringWriter w = new StringWriter();
		PrintWriter out = new PrintWriter(w);
		IRequest req = new SimpleRequest("", "word=" + s);
		try {
			org.dict.kernel.PlainPrinter.printAnswers(engine, req, a, false,
					out);
			out.flush();
		} catch (Exception e) {
	...
	private int updateDatabaseList() {
		...
		for (int i = 0; i < currentDatabases.size(); i++) {
			DatabaseConfiguration dc = (DatabaseConfiguration) currentDatabases
					.elementAt(i);
	...
	private void databaseChanged() {
		...
		scrollBar.validate();
		int count = matches.getLastVisibleIndex()
				- matches.getFirstVisibleIndex();
		if (count > 1) {
